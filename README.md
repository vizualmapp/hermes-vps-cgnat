...
| Tunnel VPS ↔ Home Assistant à travers le CGNAT | ✅ fait (NAT traversal direct validé) | [docs/04-tunnel-tailscale-cgnat.md](docs/04-tunnel-tailscale-cgnat.md) |
| Pilotage domotique par l'agent (garde-fou structurel) | ✅ fait | [docs/05-pilotage-domotique-garde-fou.md](docs/05-pilotage-domotique-garde-fou.md) |
| Guide final + relevé des coûts API | 🔜 | docs/06 à venir |
...
4. **L'agent ne reçoit jamais de jeton d'API : il reçoit un script.** Liste blanche d'actions et de cibles, validation anti-injection, périmètre physique borné. Voir le [doc 05](docs/05-pilotage-domotique-garde-fou.md).
